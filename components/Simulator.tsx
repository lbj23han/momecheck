...
function StepControl({
  ...
  const btnDisabled = isMale
    ? "bg-zinc-800 text-zinc-600 cursor-default"
    : "bg-gray-50 text-gray-300 cursor-default";
  const dropdownBg = isMale
    ? "bg-zinc-800 border border-zinc-600"
    : "bg-white border border-pink-200";
  const dropdownItem = isMale
    ? "hover:bg-zinc-700 text-zinc-200"
    : "hover:bg-pink-50 text-zinc-700";
  const dropdownActive = isMale
  ...
  return (
    <div ref={ref} className={`rounded-xl p-4 ${bg} relative`}>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <span
            className={`text-xs font-bold ${textColor} leading-snug line-clamp-2 flex-1`}
          >
            {options[value]}
          </span>
          <span className={`text-[10px] flex-shrink-0 ${labelColor}`}>
            {open ? "▲" : "▼"}
          </span>
        </button>
...
export default function Simulator({ data, baseResult, isMale }: Props) {
  ...
  const improved =
    gradeOrder.indexOf(simResult.grade) > gradeOrder.indexOf(baseResult.grade);
  ...
  return (
    <div className={`rounded-2xl p-5 ${isMale ? "bg-zinc-800" : "bg-pink-50"}`}>
      <h3
        className={`font-black text-base mb-4 ${isMale ? "text-white" : "text-zinc-800"}`}
      >
        이렇게 바꾸면 어떻게 될까? 🎛️
      </h3>
      ...
      {/* 결과 */}
      <div
        className={`rounded-xl p-4 ${isMale ? "bg-zinc-900" : "bg-white border border-pink-100"}`}
      >
        <div className="flex items-center justify-between">
          <div>
            <div className={`text-xs mb-1 ${labelColor}`}>시뮬레이션 결과</div>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black" style={{ color: point }}>
                {simResult.grade}
              </span>
              {improved && (
                <span className="text-xs text-green-400 font-bold">↑ 향상</span>
              )}
          ...
          <div className="text-right">
            {simResult.impossible ? (
              ...
              <>
                ...
                <div
                  className={`text-xl font-black ${isMale ? "text-white" : "text-zinc-800"}`}
                >
                  {simResult.dDay <= 0
                    ? "달성! 🎉"
                    : `D-${simResult.dDay.toLocaleString()}`}
                </div>
              </>
            )}
          </div>
        </div>
        <div className={`text-xs mt-2 ${labelColor}`}>
          {GRADE_COMMENTS[simResult.grade]}
        </div>
      </div>
    </div>
  );
